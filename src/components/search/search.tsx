    ...
    <form onSubmit={handleSubmit(onSearch) as () => void} >
      <Controller
        ...
        render={({ field: { onChange, value, ref } }) => (
          <input 
            className={classList(['bg-sky-800 text-slate-50 py-2 pl-2 rounded-tl rounded-bl', !!errors.location ? 'border-solid border-2 border-rose-600' : '']) } 
            onChange={onChange} 
            value={value} 
        ...
        />
      <input type="submit" className='bg-sky-800 text-slate-50 py-2 px-6 border-l border-slate-50 rounded-tr rounded-br' />
    </form>
  );
}